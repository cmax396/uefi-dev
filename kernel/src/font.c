...
// Classic IBM CP437 8x8 bitmap font. Each entry is 8 rows; MSB of each byte
// is the leftmost pixel. Entries 0x00-0x1F and 0x7F-0xFF are blank.
...
// Example for uppercase C: {0x3C,0x66,0x03,0x03,0x03,0x66,0x3C,0x00}
// 00111100
// 01100110
// 00000011
// 00000011
...
// 00111100
//
// *Assuming x=0, y=0, stride=640
//
//                 | col 0  | col 1  | col 2  | col 3  | col 4  | col 5  | col 6  | col 7  |
// fb[0:8]       = |00000000|00000000|00FFFFFF|00FFFFFF|00FFFFFF|00FFFFFF|00000000|00000000|
// fb[640:648]   = |00000000|00FFFFFF|00000000|00000000|00000000|00000000|00FFFFFF|00000000|
// fb[1280:1288] = |00FFFFFF|00FFFFFF|00000000|00000000|00000000|00000000|00000000|00000000|
// fb[1720:1728] = |00FFFFFF|00FFFFFF|00000000|00000000|00000000|00000000|00000000|00000000|

void font_draw_char(uint32_t *fb, uint32_t stride, uint32_t x, uint32_t y,
                    char symbol, uint32_t fg, uint32_t bg)
{
    const uint8_t *glyph = font8x8[(uint8_t)symbol];
    for (int row = 0; row < 8; row++) {
        uint8_t bits = glyph[row];
        for (int col = 0; col < 8; col++)
            fb[(y + (uint32_t)row) * stride + x + (uint32_t)col] = (bits & (1 << col)) ? fg : bg;
    }
}
                   ...
                   const char *string, uint32_t fg, uint32_t bg)
{
    ...
    while (*string) {
        if (*string == '\n') {
            ...
            font_draw_char(fb, stride, x_pos, y, *string, fg, bg);
            x_pos += 8;
        }
        string++;
    }
